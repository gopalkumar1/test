...
def convert_date(pvi_json):
    ...
    for i in range(len(pvi_json["products"])):
        ...
        pvi_json["products"][i]["seq_num"] = i+1
    return pvi_json
...
def conver_date_event(pvi_json):
    for idx in range(len(pvi_json["events"])):
        ...
        if pvi_json["events"][idx]["endDate"] and "/" in pvi_json["events"][idx]["endDate"]:
...
def remove_junk_value(pvi_json):
    ...
    for idx in range(len(pvi_json["tests"])):
        # for idx_test in range(len(pvi_json["tests"][idx]["testReport"])):
            if pvi_json["tests"][idx]["testResult"]:
                sample_test = pvi_json["tests"][idx]["testResult"].split()
                if sample_test[0] == sample_test[-1]:
                    pvi_json["tests"][idx]["testResult"] = sample_test[0]
...
def update_reported_reaction(pvi_json):
    for idx in range(len(pvi_json["events"])):
        if "amylase level lipase\nprice increase" in pvi_json["events"][idx]["reportedReaction"].lower():
