...
def split_rep_name(pvi_json):
    for reporter in pvi_json['reporters']:
        if reporter['country'] in (None,"") :
            if reporter['Intermediary']:
                for country in pycountry.countries:
                    if country.alpha_2 in reporter['Intermediary']:
                        print(country)
                        reporter['country'] = country.alpha_2
                    if country.alpha_3 in reporter['Intermediary']:
                        print(country)
                        reporter['country'] = country.alpha_3
        if reporter['qualification'].lower() == "healthcare professional":
            reporter['qualification'] = "Other Health Professional"
...
def convert_dates_populate_outcome(pvi_json):
    ...
    for event in pvi_json['events']:
        if event['reportedReaction']:
            ...
            event['reactionCoded'] = event['reportedReaction']
...
def fill_source_type(pvi_json):
    ...
    if source_type == "Solicited":
        pvi_json['study']['studyNumber'] = "A.S.A.P."
...
def fill_admin_sendercaseUid(pvi_json):
    ...
    if pvi_json['senderCaseVersion']:
        ...
        for reporter in pvi_json['reporters']:
            if reporter['Intermediary'] not in (None,"") and "initial" in reporter['Intermediary'].lower():
...
def pat_height_wt(pvi_json):
        ...
        if 'lbs' in pvi_json['patient']['weightUnit']:
            try:
                ...
                if weight % int(weight) >=0.5:
                    ...
                    weight = int(weight)
                pvi_json['patient']['weight'] = str(weight)
                pvi_json['patient']['weightUnit'] = 'kg'
            except:
                pass
        elif 'kg' in pvi_json['patient']['weightUnit'].lower():
            try:
                weight = float(pvi_json['patient']['weight'])
                if weight % int(weight) >=0.5:
                    weight = int(weight) +1
    ...
    if pvi_json['patient']['height'] not in (" ","", None):
        ...
        if bool(re.search(r'\d(\\)*\'\d{1,2}(\'\')*(\\)*\"*',height)):
            height_cm = float(re.split(r'\'', height)[0]) * 30.48 + float(re.split(r'\'', height)[1].strip('"')) * 2.54
        elif 'feet' in height:
            if 'inch' in height:
        ...
        elif 'cms' in height.lower() or 'cm' in height.lower():
            height_cm = height.lower().strip("cm").strip("cms")
            height_cm = int(height_cm)
        else:
            height_cm = None

        if height_cm:
            if height_cm % int(height_cm) >=0.5:
                height_cm = int(height_cm) +1
            else:
                height_cm = int(height_cm)

        pvi_json['patient']['height'] = str(height_cm)
...
def removing_repeated_string(pvi_json):
    for product in pvi_json["products"]:
        for dose in product["doseInformations"]:
            if "unk" in dose["customProperty_batchNumber_value"].lower():
                dose["customProperty_batchNumber_value"] = dose["customProperty_batchNumber_value"].lower()
                dose["customProperty_batchNumber_value"] = dose["customProperty_batchNumber_value"].replace("unk","")
            if dose["customProperty_batchNumber_value"] == "":
                dose["customProperty_batchNumber_value"] = None

            if dose["customProperty_batchNumber_value"] and dose["customProperty_batchNumber_value"] not in ("||","|"):
                fullstring = dose["customProperty_batchNumber_value"]
                givenstring = fullstring.split("||")[0].strip()
                givenstring = filter_repeated_words(givenstring)
                if "||" in fullstring:
                    dose["customProperty_batchNumber_value"] = givenstring +"||" +fullstring.split("||")[1].strip()
                else:
                    dose["customProperty_batchNumber_value"] = givenstring
    return pvi_json
